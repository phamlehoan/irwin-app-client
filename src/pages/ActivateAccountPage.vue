      ...
      <q-page class="login-page">
        <q-card class="login-card" flat bordered>
          <q-card-section class="text-center">
            ...
            <div class="text-caption text-grey-7 q-mt-xs">{{ t('app.activate_title') }}</div>
          </q-card-section>
          <q-separator />
...
async function runVerify() {
  ...
  try {
    ...
    verifyError.value = t('app.activate_invalid_token');
  } finally {
    loadingVerify.value = false;
  }
}
...
async function onSubmit() {
  ...
  const token = typeof route.query.token === 'string' ? route.query.token.trim() : '';
  if (!token) {
    submitError.value = t('app.activate_missing_token');
    return;
  }
  if (
    !runClientValidation(
  ...
  try {
    const res = await axios.post<ApiBody<{ ok: boolean }>>(
      `${apiBase}/api/v1/auth/invite/accept`,
      {
        token,
